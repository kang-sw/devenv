...
async fn server_scoped_one_shot_routes_return_bounded_refusals() {
    ...
    let phase5_scoped_cases = vec![
        ...
        (Method::GET, "/api/dashboard/servers/server-windows/work-roots/root-test/terminals", false),
        (Method::POST, "/api/dashboard/servers/server-windows/work-roots/root-test/terminals", true),
        (Method::GET, "/api/dashboard/servers/server-windows/terminals/terminal-1/output?after=7", false),
        (Method::POST, "/api/dashboard/servers/server-windows/terminals/terminal-1/input", true),
        (Method::POST, "/api/dashboard/servers/server-windows/terminals/terminal-1/resize", true),
        (Method::DELETE, "/api/dashboard/servers/server-windows/terminals/terminal-1", false),
    ];
...
#[tokio::test]
async fn linked_server_terminal_http_lifecycle_forwards_through_local_gateway() {
    ...
    let local_root = temp_fixture_path("server-scoped-terminal-local-root");
    fs::create_dir_all(&local_root).expect("create local terminal root");
    ...
    let linked = local_app
        ...
        .oneshot(
            Request::builder()
                ...
                .expect("local terminal link auth request"),
        )
        .await
        .expect("local terminal link auth response");
    assert_eq!(linked.status(), StatusCode::OK);

    let remote_work_root_id = open_work_root_via_server_for_test(
        local_app.clone(),
        cookie.as_str(),
        "server-windows",
        &remote_root,
    ...
    .await;
    let local_work_root_id =
        open_work_root_for_test(local_app.clone(), cookie.as_str(), &local_root).await;
    let local_terminal_id =
        create_terminal_for_test(local_app.clone(), cookie.as_str(), &local_work_root_id).await;
    ...
    let create = local_app
        ...
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri(format!(
                    "/api/dashboard/servers/server-windows/work-roots/{remote_work_root_id}/terminals"
                ))
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                    serde_json::json!({ "columns": 80, "rows": 24, "title": "Remote terminal" })
                        .to_string(),
                ))
                .expect("server scoped terminal create request"),
        )
        .await
        .expect("server scoped terminal create response");
    ...
    let create_body = axum::body::to_bytes(create.into_body(), 4096)
        .await
        .expect("server scoped terminal create body");
    let created: serde_json::Value =
        serde_json::from_slice(&create_body).expect("server scoped terminal create JSON");
    let remote_terminal_id = created["terminalId"]
        .as_str()
        .expect("remote terminal id")
        .to_owned();
    assert_eq!(created["workRootId"], remote_work_root_id);
    ...
    let list = local_app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server scoped terminal list request"),
        )
        .await
        .expect("server scoped terminal list response");
    assert_eq!(list.status(), StatusCode::OK);
    let list_body = axum::body::to_bytes(list.into_body(), 4096)
        .await
        .expect("server scoped terminal list body");
    let listed: serde_json::Value =
        serde_json::from_slice(&list_body).expect("server scoped terminal list JSON");
    assert_eq!(listed.as_array().expect("terminal list").len(), 1);
    assert_eq!(listed[0]["terminalId"], remote_terminal_id);
    ...
    let output = local_app
        ...
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/dashboard/servers/server-windows/terminals/{remote_terminal_id}/output?after=7"
                ))
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server scoped terminal output request"),
        )
        .await
        .expect("server scoped terminal output response");
    ...
    let input = local_app
        ...
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri(format!(
                    "/api/dashboard/servers/server-windows/terminals/{remote_terminal_id}/input"
                ))
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(serde_json::json!({ "data": "" }).to_string()))
                .expect("server scoped terminal input request"),
        )
        .await
        .expect("server scoped terminal input response");
    assert_eq!(input.status(), StatusCode::NO_CONTENT);

    let resize = local_app
        .clone()
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri(format!(
                    "/api/dashboard/servers/server-windows/terminals/{remote_terminal_id}/resize"
                ))
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                ...
                .expect("server scoped terminal resize request"),
        )
        .await
        .expect("server scoped terminal resize response");
    assert_eq!(resize.status(), StatusCode::OK);
    let resize_body = axum::body::to_bytes(resize.into_body(), 4096)
        .await
        .expect("server scoped terminal resize body");
    let resized: serde_json::Value =
        serde_json::from_slice(&resize_body).expect("server scoped terminal resize JSON");
    ...
    let close = local_app
        ...
        .oneshot(
            Request::builder()
                .method(Method::DELETE)
                .uri(format!(
                    "/api/dashboard/servers/server-windows/terminals/{remote_terminal_id}"
                ))
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server scoped terminal close request"),
        )
        .await
        .expect("server scoped terminal close response");
    assert_eq!(close.status(), StatusCode::NO_CONTENT);

    let remote_after_close = local_app
        .clone()
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/dashboard/servers/server-windows/terminals/{remote_terminal_id}/output"
                ))
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server scoped terminal output after close request"),
        )
        .await
        .expect("server scoped terminal output after close response");
    assert_eq!(remote_after_close.status(), StatusCode::NOT_FOUND);

    let local_still_alive = local_app
        .clone()
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/dashboard/terminals/{local_terminal_id}/output?after=0"
                ))
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("local terminal output after remote close request"),
        )
        .await
        .expect("local terminal output after remote close response");
    assert_eq!(local_still_alive.status(), StatusCode::OK);

    let _ = local_app
        .oneshot(
            Request::builder()
                .method(Method::DELETE)
                .uri(format!("/api/dashboard/terminals/{local_terminal_id}"))
                .header(header::COOKIE, cookie)
                .body(Body::empty())
                .expect("local terminal cleanup request"),
        )
        .await
        .expect("local terminal cleanup response");
    remote_server.abort();
    remove_static_fixture(&remote_root);
...
struct TerminalForwardingProbe {
    calls: Arc<Mutex<Vec<ObservedTerminalForwardingCall>>>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
struct ObservedTerminalForwardingCall {
    method: String,
...
#[derive(Clone, Default)]
struct TerminalWebSocketForwardingProbe {
    upgrades: Arc<Mutex<Vec<ObservedTerminalForwardingCall>>>,
    frames: Arc<Mutex<Vec<String>>>,
    closed: Arc<Notify>,
}

async fn mock_terminal_link_auth() -> impl IntoResponse {
    (
        StatusCode::OK,
        [(header::CONTENT_TYPE, "application/json")],
        serde_json::json!({ "bearerToken": "linked-terminal-token" }).to_string(),
    )
}

async fn mock_terminal_websocket_forwarding(
    axum::extract::State(probe): axum::extract::State<TerminalWebSocketForwardingProbe>,
    ...
    probe
        ...
        .lock()
        .await
        .push(ObservedTerminalForwardingCall {
            method: "GET".to_owned(),
...
async fn mock_terminal_input_forwarding(
    axum::extract::State(probe): axum::extract::State<TerminalForwardingProbe>,
    method: Method,
    uri: axum::extract::OriginalUri,
    headers: axum::http::HeaderMap,
    body: Bytes,
    ...
    probe
        ...
        .push(ObservedTerminalForwardingCall {
            ...
            content_type: headers
                ...
                .and_then(|value| value.to_str().ok())
                .map(str::to_owned),
            body: String::from_utf8(body.to_vec()).expect("forwarded input body is UTF-8"),
        });
    StatusCode::NO_CONTENT
}

async fn mock_terminal_output_upstream_error(
    axum::extract::State(probe): axum::extract::State<TerminalForwardingProbe>,
    method: Method,
    uri: axum::extract::OriginalUri,
    headers: axum::http::HeaderMap,
    ...
    (
        ...
        serde_json::json!({ "error": "upstream terminal missing" }).to_string(),
    )
}

#[tokio::test]
async fn linked_server_terminal_input_forwards_bearer_body_and_preserves_upstream_errors() {
    let probe = TerminalForwardingProbe::default();
    let remote_app = axum::Router::new()
        ...
        .route(
            "/api/dashboard/terminals/term-input/input",
            axum::routing::post(mock_terminal_input_forwarding),
        )
        .route(
            "/api/dashboard/terminals/term-error/output",
            axum::routing::get(mock_terminal_output_upstream_error),
        )
        .with_state(probe.clone());
    let (remote_addr, remote_server) = spawn_test_server(remote_app).await;

    let state_file_root = temp_fixture_path("server-scoped-terminal-input-forwarding-state");
    let store = DashboardStateStore::at_path(state_file_root.join("opened-workroots.json"));
    store
        .persist_linked_servers(vec![PersistedLinkedServer {
            id: ServerId::from("server-windows"),
    ...
    let linked = local_app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                    serde_json::json!({ "passphrase": "mock-passphrase" }).to_string(),
                ))
                .expect("mock terminal link auth request"),
        )
        .await
        .expect("mock terminal link auth response");
    assert_eq!(linked.status(), StatusCode::OK);
    ...
    let input = local_app
        ...
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri("/api/dashboard/servers/server-windows/terminals/term-input/input")
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json; charset=utf-8")
                .body(Body::from(
                    serde_json::json!({ "data": "echo forwarded-input\n" }).to_string(),
                ))
                .expect("server scoped terminal input forwarding request"),
        )
        .await
        .expect("server scoped terminal input forwarding response");
    assert_eq!(input.status(), StatusCode::NO_CONTENT);

    let input_call = probe
        .calls
        .lock()
        .await
        .iter()
        .find(|call| call.path_and_query == "/api/dashboard/terminals/term-input/input")
        .cloned()
        .expect("terminal input forwarding call");
    assert_eq!(input_call.method, "POST");
    assert_eq!(
        input_call.authorization.as_deref(),
        Some("Bearer linked-terminal-token")
    );
    assert_eq!(
        input_call.content_type.as_deref(),
        Some("application/json; charset=utf-8")
    );
    assert_eq!(
        serde_json::from_str::<serde_json::Value>(&input_call.body)
            .expect("forwarded input JSON body"),
        serde_json::json!({ "data": "echo forwarded-input\n" })
    );

    let upstream_error = local_app
        .oneshot(
            Request::builder()
                .uri("/api/dashboard/servers/server-windows/terminals/term-error/output?after=9")
                .header(header::COOKIE, cookie)
                .body(Body::empty())
                .expect("server scoped terminal output upstream error request"),
        )
        .await
        .expect("server scoped terminal output upstream error response");
    ...
    let error_body = axum::body::to_bytes(upstream_error.into_body(), 4096)
        .await
        .expect("upstream terminal error body");
    assert_eq!(
        serde_json::from_slice::<serde_json::Value>(&error_body)
            .expect("upstream terminal error JSON"),
        serde_json::json!({ "error": "upstream terminal missing" })
    );
    let output_call = probe
        .calls
        .lock()
        .await
        .iter()
        .find(|call| call.path_and_query == "/api/dashboard/terminals/term-error/output?after=9")
        .cloned()
        .expect("terminal output error forwarding call");
    assert_eq!(output_call.method, "GET");
    assert_eq!(
        output_call.authorization.as_deref(),
        Some("Bearer linked-terminal-token")
    );
...
#[tokio::test]
async fn linked_server_terminal_websocket_gateways_bearer_frames_and_cleanup() {
    ...
    let state_file_root = temp_fixture_path("server-scoped-terminal-websocket-forwarding-state");
    let store = DashboardStateStore::at_path(state_file_root.join("opened-workroots.json"));
    store
        .persist_linked_servers(vec![PersistedLinkedServer {
            id: ServerId::from("server-windows"),
        ...
        .await
        .expect("persist linked terminal websocket server seed");
    let local_state = app_state_with_opened_and_store(OpenedWorkRoots::default(), store);
    let token = local_state
        .auth
        .pairing_token()
    ...
    let unauthenticated_request =
        format!("ws://{local_addr}/api/dashboard/servers/server-windows/terminals/term-live/socket")
            .into_client_request()
            .expect("unauthenticated linked terminal websocket request");
    let error = tokio_tungstenite::connect_async(unauthenticated_request)
        .await
        .expect_err("unauthenticated linked terminal websocket rejects");
    match error {
        tokio_tungstenite::tungstenite::Error::Http(response) => {
            assert_eq!(response.status(), StatusCode::UNAUTHORIZED);
        }
        other => panic!("unexpected unauthenticated websocket error: {other}"),
    }

    let mut auth_required_request =
        format!("ws://{local_addr}/api/dashboard/servers/server-windows/terminals/term-live/socket")
            .into_client_request()
            .expect("auth-required linked terminal websocket request");
    auth_required_request
        .headers_mut()
        .insert(header::COOKIE, cookie.parse().expect("cookie header"));
    let error = tokio_tungstenite::connect_async(auth_required_request)
        .await
        .expect_err("auth-required linked terminal websocket rejects");
    match error {
        tokio_tungstenite::tungstenite::Error::Http(response) => {
            assert_eq!(response.status(), StatusCode::CONFLICT);
        }
        other => panic!("unexpected auth-required websocket error: {other}"),
    }
    ...
    let linked = local_app
        ...
        .oneshot(
            Request::builder()
                ...
                .expect("mock terminal websocket link auth request"),
        )
        .await
        .expect("mock terminal websocket link auth response");
    assert_eq!(linked.status(), StatusCode::OK);

    let mut request =
        format!("ws://{local_addr}/api/dashboard/servers/server-windows/terminals/term-live/socket?after=41")
            .into_client_request()
            .expect("linked terminal websocket request");
    request
        .headers_mut()
        .insert(header::COOKIE, cookie.parse().expect("cookie header"));
    let (mut socket, response) = tokio_tungstenite::connect_async(request)
        .await
        .expect("connect linked terminal websocket");
    assert_eq!(response.status(), StatusCode::SWITCHING_PROTOCOLS);

    let first = timeout(Duration::from_secs(2), socket.next())
        .await
        .expect("linked websocket output timeout")
        .expect("linked websocket output")
        .expect("linked websocket frame");
    let TungsteniteMessage::Text(first_text) = first else {
        panic!("expected linked websocket text output");
    };
    assert_eq!(
        serde_json::from_str::<serde_json::Value>(&first_text).expect("linked ws JSON"),
        serde_json::json!({
            "type": "output",
    ...
    socket
        .send(TungsteniteMessage::Text(input_frame.clone().into()))
        .await
        .expect("send linked websocket input");
    socket
        .send(TungsteniteMessage::Text(resize_frame.clone().into()))
        .await
        .expect("send linked websocket resize");
    socket
    ...
    timeout(Duration::from_secs(2), probe.closed.notified())
        .await
        .expect("upstream websocket cleanup");
    ...
    match error {
        ...
        other => panic!("unexpected linked terminal upstream rejection: {other}"),
    }

    let mut upstream_close_request =
        format!("ws://{local_addr}/api/dashboard/servers/server-windows/terminals/term-upstream-close/socket")
            .into_client_request()
            .expect("linked terminal upstream close websocket request");
    upstream_close_request
        .headers_mut()
        .insert(header::COOKIE, cookie.parse().expect("cookie header"));
    let (mut upstream_close_socket, response) =
    ...
    let close = timeout(Duration::from_secs(2), upstream_close_socket.next())
        .await
        .expect("upstream close propagated timeout")
        .expect("upstream close propagated")
...
async fn open_work_root_via_server_for_test(
    app: axum::Router,
    cookie: &str,
    server_id: &str,
    root: &Path,
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                .expect("server scoped open workRoot request"),
        )
        .await
        .expect("server scoped open workRoot response");
    assert_eq!(response.status(), StatusCode::OK);
    response
        .headers()
        .get("x-ws-dashboard-opened-work-root-id")
        .and_then(|value| value.to_str().ok())
        .map(str::to_owned)
        .expect("server scoped opened workRoot id header")
}
...
#[tokio::test]
async fn server_scoped_terminal_http_routes_dispatch_local_aliases_and_registers_socket() {
    let root = temp_fixture_path("server-scoped-local-terminal-root");
    fs::create_dir_all(&root).expect("create server-local terminal root");
    let state = app_state();
    let token = state.auth.pairing_token().expose_for_owner_url().to_owned();
    let app = build_router(state);
    let cookie = pair_and_cookie(app.clone(), &token).await;
    ...
    let create = app
        ...
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri(format!(
                    "/api/dashboard/servers/server-local/work-roots/{work_root_id}/terminals"
                ))
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                    serde_json::json!({ "columns": 80, "rows": 24, "title": "Server-local terminal" })
                        .to_string(),
                ))
                .expect("server-local terminal create request"),
        )
        .await
        .expect("server-local terminal create response");
    assert_eq!(create.status(), StatusCode::OK);
    let create_body = axum::body::to_bytes(create.into_body(), 4096)
        .await
        .expect("server-local terminal create body");
    let created: serde_json::Value =
        serde_json::from_slice(&create_body).expect("server-local terminal create JSON");
    let terminal_id = created["terminalId"]
        .as_str()
        .expect("server-local terminal id")
        .to_owned();
    assert_eq!(created["workRootId"], work_root_id);
    ...
    let list = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server-local terminal list request"),
        )
        .await
        .expect("server-local terminal list response");
    assert_eq!(list.status(), StatusCode::OK);
    let list_body = axum::body::to_bytes(list.into_body(), 4096)
        .await
        .expect("server-local terminal list body");
    let listed: serde_json::Value =
        serde_json::from_slice(&list_body).expect("server-local terminal list JSON");
    assert_eq!(
        listed.as_array().expect("server-local terminal list").len(),
        1
    );
    assert_eq!(listed[0]["terminalId"], terminal_id);

    let initial_output = app
        .clone()
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/dashboard/servers/server-local/terminals/{terminal_id}/output?after=0"
                ))
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server-local terminal output request"),
        )
        .await
        .expect("server-local terminal output response");
    assert_eq!(initial_output.status(), StatusCode::OK);
    ...
    let resize = app
        ...
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri(format!(
                    "/api/dashboard/servers/server-local/terminals/{terminal_id}/resize"
                ))
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                ...
                .expect("server-local terminal resize request"),
        )
        .await
        .expect("server-local terminal resize response");
    assert_eq!(resize.status(), StatusCode::OK);
    let resize_body = axum::body::to_bytes(resize.into_body(), 4096)
        .await
        .expect("server-local terminal resize body");
    let resized: serde_json::Value =
        serde_json::from_slice(&resize_body).expect("server-local terminal resize JSON");
    assert_eq!(resized["columns"], 100);
    assert_eq!(resized["rows"], 30);
    ...
    let input = app
        ...
        .oneshot(
            Request::builder()
                .method(Method::POST)
                .uri(format!(
                    "/api/dashboard/servers/server-local/terminals/{terminal_id}/input"
                ))
                .header(header::COOKIE, cookie.as_str())
                .header(header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                    serde_json::json!({ "data": terminal_test_commands_for_current_platform("server-local-alias").echo_and_exit })
                        .to_string(),
                ))
                .expect("server-local terminal input request"),
        )
        .await
        .expect("server-local terminal input response");
    assert_eq!(input.status(), StatusCode::NO_CONTENT);
        ...
        let response = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header(header::COOKIE, cookie.as_str())
                    .body(Body::empty())
                    .expect("server-local terminal output poll request"),
            )
            .await
            .expect("server-local terminal output poll response");
        assert_eq!(response.status(), StatusCode::OK);
        let body = axum::body::to_bytes(response.into_body(), 64 * 1024)
            .await
            .expect("server-local terminal output body");
        let value: serde_json::Value =
            serde_json::from_slice(&body).expect("server-local terminal output JSON");
        output_text = value["chunks"]
            .as_array()
            .expect("chunks array")
            .iter()
            ...
            .collect::<Vec<_>>()
            .join("");
        if output_text.contains("server-local-alias") {
            break;
        }
        tokio::time::sleep(Duration::from_millis(50)).await;
    }
    assert!(
        output_text.contains("server-local-alias"),
        "{output_text:?}"
    );
    ...
    let close = app
        ...
        .oneshot(
            Request::builder()
                .method(Method::DELETE)
                .uri(format!(
                    "/api/dashboard/servers/server-local/terminals/{terminal_id}"
                ))
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server-local terminal close request"),
        )
        .await
        .expect("server-local terminal close response");
    assert_eq!(close.status(), StatusCode::NO_CONTENT);

    for (uri, body) in [
        (
            "/api/dashboard/servers/server-local/work-roots/root-local-missing/terminals",
            serde_json::json!({ "columns": 80, "rows": 24 }).to_string(),
        ),
        (
            "/api/dashboard/servers/server-local/terminals/term-missing/input",
            serde_json::json!({ "data": "x" }).to_string(),
        ),
        (
            "/api/dashboard/servers/server-local/terminals/term-missing/resize",
            serde_json::json!({ "columns": 80, "rows": 24 }).to_string(),
        ),
    ] {
        let missing_content_type = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header(header::COOKIE, cookie.as_str())
                    .body(Body::from(body))
                    .expect("server-local terminal JSON boundary request"),
            )
            .await
            .expect("server-local terminal JSON boundary response");
        assert_eq!(
            missing_content_type.status(),
    ...
    let list_unknown = app
        .clone()
        .oneshot(
            Request::builder()
                .uri("/api/dashboard/servers/server-local/work-roots/root-local-missing/terminals")
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server-local terminal list request"),
        )
        .await
        .expect("server-local terminal list response");
    assert_eq!(list_unknown.status(), StatusCode::NOT_FOUND);
    let body = axum::body::to_bytes(list_unknown.into_body(), 4096)
        .await
        .expect("server-local terminal list body");
    let value: serde_json::Value =
        serde_json::from_slice(&body).expect("server-local terminal list JSON");
    assert_eq!(value["error"], "unknown workRoot");

    let output_unknown = app
        .clone()
        .oneshot(
            Request::builder()
                .uri("/api/dashboard/servers/server-local/terminals/term-missing/output?after=7")
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server-local terminal output request"),
        )
        .await
        .expect("server-local terminal output response");
    assert_eq!(output_unknown.status(), StatusCode::NOT_FOUND);

    let close_unknown = app
        .clone()
        .oneshot(
            Request::builder()
                .method(Method::DELETE)
                .uri("/api/dashboard/servers/server-local/terminals/term-missing")
                .header(header::COOKIE, cookie.as_str())
                .body(Body::empty())
                .expect("server-local terminal close request"),
        )
        .await
        .expect("server-local terminal close response");
    assert_eq!(close_unknown.status(), StatusCode::NOT_FOUND);

    let socket_without_upgrade = app
        .oneshot(
            Request::builder()
                .uri("/api/dashboard/servers/server-local/terminals/term-missing/socket")
                .header(header::COOKIE, cookie)
                .body(Body::empty())
                .expect("server-scoped terminal socket request"),
        )
        .await
        .expect("server-scoped terminal socket response");
    assert_eq!(socket_without_upgrade.status(), StatusCode::BAD_REQUEST);
    remove_static_fixture(&root);
}
