...
/// <callback_path> --state <state>`, both paths single-quoted so a
/// space-containing state-dir path does not break the vendor CLI's shell
/// parsing of the hook command (mirrors `claude_cli.rs`'s
/// `shell_single_quote`, duplicated here since that helper is private to its
/// own module).
///
...
pub fn materialize_hook_config(
    ...
    let notify_binary_quoted = shell_single_quote(&notify_binary.display().to_string());
    let callback_path_quoted = shell_single_quote(&callback_path.display().to_string());
