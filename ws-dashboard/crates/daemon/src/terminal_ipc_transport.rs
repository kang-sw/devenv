...
// to the caller.
//
// CONTRACT (260729 review round 4): this pattern is only sound while
// `accept()` is ALWAYS polled, because "armed" is conditional on the poll.
// A previous revision of this ticket briefly held a probe-slot permit BEFORE
// calling `accept()`, which deliberately stopped polling it under load; on
// Windows that left no unconnected instance, so the next daemon to connect
// got `ERROR_PIPE_BUSY` and its outcome inverted into a kill. That was fixed
// here with a pre-armed pool polled via `select_all`, and both halves are now
// withdrawn: `terminal_helper_process::serve_session` polls `accept()`
// unconditionally again (the probe bound was deleted), so the standard
// single-instance pattern holds. Do not stop polling `accept()` without
// restoring a real backlog here first.
#[cfg(windows)]
