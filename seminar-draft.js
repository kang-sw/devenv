...
const children = [
  ...
  // ── Part 5 워크플로우 체인 개요 ──
  h1("Part 5. 워크플로우 체인"),
  p("원칙의 구체적 구현. 핵심: 아티팩트가 인터페이스."),
  bullet("각 단계: 이전 아티팩트를 입력으로 받아 다음 아티팩트를 출력으로 생성."),
  bullet("단계 간 독립성: 컨텍스트 리셋 후에도 아티팩트만으로 올바른 재개 가능."),
  bullet("전체 흐름: /discuss → /write-ticket → /write-plan → 실행 경로."),
  spacer(),

  // ── /discuss ──
  h1("/discuss — 발산의 공간"),
  h2("역할"),
  bullet("아이디어 탐색, 방향 설정, 리스크 탐지의 단계."),
  bullet("mental-model 문서 참조 — 소스 코드 직접 탐색 금지."),
  bullet("필요 시 Explore 서브에이전트 위임으로 구체적 코드 확인."),
  h2("출력"),
  bullet("이 단계의 직접 아티팩트: 없음."),
  bullet("결과물: /write-ticket을 위한 명확한 방향과 합의된 접근."),
  spacer(),

  // ── /write-ticket ──
  h1("/write-ticket — 결정의 외재화"),
  h2("역할"),
  bullet("티켓 = 의도를 잠그는 아티팩트. 프로젝트의 결정 원장."),
  h2("담기는 내용"),
  bullet("목표와 배경 — 왜 이 작업인가."),
  bullet("거부된 대안과 거부 이유 — 재검토 방지."),
  bullet("합의된 접근과 제약·경계 조건."),
  bullet("구현 구조 스케치 (소스 탐색 없이 — 탐색은 /write-plan 단계)."),
  h2("자기완결성 기준"),
  bullet("품질 테스트: 대화 미참여 에이전트가 티켓만으로 다음 단계 진행 가능한가."),
  h2("상태 관리"),
  bullet("파일 이동 기반: idea/ → todo/ → wip/ → done/."),
  bullet("참조는 줄기(stem)만 — 상태 이동 시 경로 변경에 무관."),
  spacer(),

  // ── /write-plan ──
  h1("/write-plan — 코드베이스와의 연결"),
  h2("역할"),
  bullet("티켓(무엇을) → 계획서(어떻게): 실제 코드베이스에 맞춘 구체화."),
  bullet("신선한 컨텍스트 서브에이전트로 가정 검증 — fresh eyes 원칙."),
  h2("포함 내용"),
  bullet("변경 파일 목록과 각 변경의 계약 (입력/출력/부작용)."),
  bullet("통합 지점, 테스트 전략."),
  h2("영구화"),
  bullet("완성 후 코드 저장소에 커밋 — 불변 아티팩트."),
  bullet("자기완결성 기준: 이전 대화 없이 실행 에이전트가 계획서만으로 진행 가능한가."),
  spacer(),

  // ── 실행 경로 ──
  h1("실행 경로"),
  h2("/execute-plan"),
  bullet("계획서를 계약으로 충실히 구현 — 임의 재설계 금지."),
  bullet("가정≠실제 발견 시: 정지 후 보고. 자의적 수정 금지."),
  bullet("적합: 대규모 변경, 높은 정밀도 요구."),
  h2("/implement"),
  bullet("계획서가 방향과 파일 목록만 제시하는 경우의 유연한 구현."),
  bullet("적합: 익숙한 패턴, 세부 계약 없이 탐색 기반 진행 가능한 작업."),
  h2("/sprint"),
  ...
  bullet("Explore: 저렴한 모델(Haiku)로 시작, 부족 시 에스컬레이션."),
