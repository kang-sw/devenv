...
5. **Explore when needed**: Use focused search and reads for local queries. For broader codebase questions, call `ws/subquery(deep_research: false)`, then `ws/agents.wait(name: <subquery-key>, timeout_seconds: 600)`; use `deep_research: true` only for broad tracing.
