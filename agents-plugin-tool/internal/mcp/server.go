...
func (s *Server) ServeStdio(ctx context.Context, in io.Reader, out io.Writer) error {
	...
	// every return path below cancels it and waits for its goroutine. At
	// EOF it stops before in-flight handlers drain, so a disconnected
	// client's name is not kept live by a slow handler.
	tickerCtx, stopTicker := context.WithCancel(ctx)
	tickerDone := s.startMailboxPresenceTicker(tickerCtx)
	stopPresenceTicker := func() {
		stopTicker()
		<-tickerDone
	}
...
func sageGateNextInstruction(result wsdoc.SageGateResult) string {
	...
	case "stop_open_decision_queue":
		...
		return "next_instruction: This ticket still has a ## Open Decision Queue section, whose items are unsettled working state rather than decisions, so it cannot be reviewed or promoted yet. Settle the queue with the user (announced defaults acknowledged, every policy question settled), move each settled item into its home section, delete the section, then call tickets.sage_gate again with the same stem/landing."
	case "ask":
