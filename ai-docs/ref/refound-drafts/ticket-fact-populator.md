...
A table under `## Route Facts`, placed immediately before `## Phases`:

```
| fact | value | evidence |
|---|---|---|
| scope.span | single-file \| multi-file \| cross-module | <paths the ticket names> |
| scope.surface | internal \| public | <exported symbol or none> |
| scope.test_surface | existing \| new-files \| none | <test path or search> |
| risk.correctness | low \| medium \| high | <one clause> |
| risk.fit | low \| medium \| high | <one clause> |
| risk.test | low \| medium \| high | <one clause> |
| risk.security_or_contract | low \| medium \| high | <one clause> |
```

Values come from the ticket's phase text and the tree: the files it names,
whether they are exported surface, whether tests cover them. A value you
cannot ground is `unknown` with the reason in the evidence column; never
guess a `low`.

[design-review: the surviving fact set and enum values are fixed by the
route-facts child's Phase 1 Result; the heading, placement, table shape, and
evidence column are this draft's proposal.]
