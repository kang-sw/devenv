...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.37.6`. |
...
documentation references. Development binaries such as `0.37.6-dev` satisfy the
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.37.6-dev
