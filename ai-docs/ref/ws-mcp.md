...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.16.3`. |
...
The current binary reports `0.16.3-dev`. The runtime contract lets launcher
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.16.3-dev
