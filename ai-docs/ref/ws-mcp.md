...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.20.2`. |
...
The current binary reports `0.20.2-dev`. The runtime contract lets launcher
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.20.2-dev
