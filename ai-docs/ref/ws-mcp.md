...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.36.11`. |
...
documentation references. Development binaries such as `0.36.11-dev` satisfy
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.36.11-dev
