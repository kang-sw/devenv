...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.23.3`. |
...
The current binary reports `0.23.3-dev`. The runtime contract lets launcher
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.23.3-dev
