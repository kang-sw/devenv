...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.20.1`. |
...
The current binary reports `0.20.1-dev`. The runtime contract lets launcher
...
checks an existing cache-local binary first; compatible `0.20.x` binaries run
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.20.1-dev
