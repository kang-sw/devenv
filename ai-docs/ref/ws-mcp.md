...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.21.2`. |
...
The current binary reports `0.21.2-dev`. The runtime contract lets launcher
...
checks an existing cache-local binary first; compatible `0.21.x` binaries run
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.21.2-dev
