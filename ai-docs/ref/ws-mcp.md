...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.17.2`. |
...
The current binary reports `0.17.2-dev`. The runtime contract lets launcher
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.17.2-dev
