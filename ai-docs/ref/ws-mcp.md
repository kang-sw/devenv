...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.29.1`. |
...
documentation references. Development binaries such as `0.29.1-dev` satisfy
plugin `0.29.1`; older or newer minor releases are stale.
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.29.1-dev
