...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.16.0`. |
...
The current binary reports `0.16.0-dev`. Phase 3 must add a runtime contract that
...
checks an existing cache-local binary first; compatible `0.16.x` binaries run
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.16.0-dev
