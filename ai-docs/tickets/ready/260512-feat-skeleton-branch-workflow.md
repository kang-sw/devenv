...
- `lead-proceed` routes implementation-ready work to `lead-implement`; it does
  not decide whether skeleton work is needed and does not invoke
  `lead-write-skeleton`.
- `lead-implement` owns implementation branch setup and skeleton need
  resolution before skeleton or implementation edits run.
...
- revise `lead-proceed` pipeline wording and execution steps so it routes only
  implementation-ready targets into `lead-implement`;
- revise `lead-implement` task flow so it decides skeleton need and creates
  delegated implementation branches before optional skeleton writing, while
  direct-edit mode can still run on the current branch;
...
- `lead-proceed` no longer invokes or decides `lead-write-skeleton` directly.
- `lead-implement` documents optional skeleton decision and execution before
  edit/write-code.
