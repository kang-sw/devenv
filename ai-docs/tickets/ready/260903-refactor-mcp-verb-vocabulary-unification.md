...
tokens (`agents-plugin/rsrc/` and the wsflow mirror — regenerated by the mirror
generator in `agents-plugin-tool/internal/wsrsrc/`, drift-guarded by
`wsflow_mirror_test.go`; procedure in `ai-docs/manuals/wsflow-mirroring.md`), and
tests. Verify by diff + full test run (`go test ./...` in `agents-plugin-tool/`). Only a thin prose-cleanup tail
...
tokens in `agents-plugin/rsrc/` and the wsflow mirror (regenerated via the mirror
generator in `agents-plugin-tool/internal/wsrsrc/`, not hand-edited), and tests.
Then a thin human prose-cleanup pass for surrounding
...
grep sweep is clean), the full Go test suite is green (`go test ./...` in
`agents-plugin-tool/`), and `tools/list` shows the canonical verbs. Behavior is
