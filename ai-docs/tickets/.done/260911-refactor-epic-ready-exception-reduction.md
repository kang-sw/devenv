...
### Result (b444a501) - 2026-09-11

Landed the hard ready/ bar and preserved the decoupled epic-at-ready design-only
gate.

- `tickets_mutate.go`: `TicketsMove` now rejects any `nonImplementationCategories`
  member (`epic`, `research`, `workset`) at `to == "ready"`, placed before any
  write so the rejection is a genuine no-op (no git call, source file unmoved).
  The bar reads the existing `nonImplementationCategories` set, whose comment now
  records that TicketsMove reads it as the ready-landing bar. `sageReviewStageRequirement`
  is unchanged.
- `tickets_sage.go`: the `designRequired && !completenessRequired` epic-at-ready
  branch is retained with a refreshed rationale comment — `sage_gate` is
  decoupled from `tickets.move`, so a direct `sage_gate(epic, landing: "ready")`
  stays reachable and must remain design-only (never fall into the both-stages
  completeness path). No logic change.
- `server.go`: `tickets.move` tool description documents the non-implementation
  category bar.
- Tests: replaced the three now-unreachable epic/research/workset ready-move
  success tests with `TestTicketsMoveToReadyBarsNonImplementationCategories`
  (asserts error naming `ready/`, no git call, source ticket unmoved); dropped
  the exempt-category case from `TestTicketsMoveReadyStaysSilentWhenFactsAreNotOwed`
  (that path is now barred; the route-facts exemption's live coverage is the
  direct `sage_gate` path in `tickets_sage_test.go`). Existing
  `TestSageGateCategoryMatrix` still pins the retained epic-at-ready design-only
  behavior.

Decisions: dropped the exempt-category test case rather than repurposing it,
since non-implementation categories can no longer reach the ready path in
TicketsMove.

Verification: `go test ./...` (agents-plugin-tool) — ok, all packages.

Review (test partition, Important, fixed): the removed exempt-category case left
`missingRouteFacts`'s non-implementation category early-return untested (the
`sage_gate` fixtures always carry a Route Facts section). Added
`TestSageGateReadyNonImplementationCategoriesSkipMissingRouteFacts`
(research/workset skip, epic runs design standalone — none refused with
stop_missing_route_facts) and corrected the route-facts test comment to name it.
...
### Result (95c90d59) - 2026-09-11

Reframed the epic prose across the four shipped surfaces and added the interim
lead-run nudge, mirrored into wsflow.

- `lead-ticket.md`: renamed "Settle epic design" -> "Review epic design",
  rewritten to the soft-living-board / lead-judgment-invoked model (epic never an
  execution target, never enters `ready/`; design review run on judgment when the
  cross-child design has drifted, not as a boundary). The Go entry point stays
  `sage_gate(stem, landing: "todo")`. The "Ground: fact population" trigger and
  the "Epics and research stay in idea/todo" line were reframed to match.
- `ticket-conventions.md`, `lead-workflow-manual.md`: same reframe.
- `server.go`: `sage_gate` description and `landing` enum re-narrate the todo
  landing as lead-judgment-invoked epic design-only review, not a boundary.
- `lead-run.md`: one interim line at the terminal — when the closed ticket was an
  epic's last open child, surface that epic to the user for a close decision. The
  robust reliable-trigger version is deferred to the separate research ticket;
  per the shipped-surface boundary the shipped line references it only
  generically (no ticket stem).
- Mirror: the three `rsrc/` edits regenerated byte-identically into
  `agents-plugin-wsflow/` via `WSRSRC_REGEN` + `WS_REGEN_WSFLOW_RSRC`; both
  manifests regenerated. Never hand-edited.

Decisions: `TestLeadTicketSettlementBoundaries` ground check switched to
whitespace-normalized matching (matching its own heading-section checks) so it
stops pinning physical line wraps, which broke on the prose reflow; pinned
strings and the heading updated to the reframed text.

Verification: `go test ./... -count=1` (agents-plugin-tool) — ok, all packages;
`python3 -m unittest discover agents-plugin-wsflow/tests` — OK (10 tests);
`go test ./internal/wsrsrc/... -count=1` (byte-identity + manifest drift guards)
— ok.

Review (correctness partition — clean; fit partition — clean, one Minor fixed:
the epic-close nudge was folded into the lead-run `stop: none` clause it applies
to instead of a paragraph sandwiched before the stop-letter list).
