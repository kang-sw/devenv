...
### Result (96604020) - 2026-08-23

Caller-visible delta: the ten per-knob `config.*` tools are gone; callers read
through `config.list` and write through `config.tune("<key>", <value>[, scope][,
harness])`. Landed as one atomic commit-set (`104831df` impl + `96604020` review
fix), 36 files.

- `config.list` subsumes `config.show` (resolved View + resolved-from scope) and
  `config.tuning` (per-key catalog, `format:"json"` shape, no-agent full-ws-only
  omission) — both source contracts preserved in one payload (`configListView`
  embeds `wsconfig.View` + adds `knobs`).
- `config.tune` is the sole writer, dispatched by a new `resolveConfigEntryForKey`
  (keyed by config key, with `prompt.*` prefix handling). It handles scalar knobs
  (`resolver.Set`, verbatim report text), `prompt.*` set/unset (`*`→`all`
  normalization), and the `agents.tier` compound value `{tier,backend,model,effort}`
  via `SetAgentsTierForHarness`; `reset:true` routes only where `ResetTool != ""`.
- Decision 9: registry edited so `tier` moved from `SelectorFields` into
  `ValueFields` (required); `SelectorFields` = `harness` only.
- Gating preserved per-key at dispatch: no-agent hiding (`NoAgentMode() &&
  !NoAgentVisible` rejects `workflow.prefer_mercenary`), authority (lead-authority
  keys → `requireLeadSessionKey`), scope routing (resolver enforces global-only +
  session-key for free; `agents.tier` rejects non-project scope).

Decisions taken that plan/ticket did not settle verbatim:
- `session_key` is no longer a schema-static `required` field on any config write
  (one generic tool cannot statically vary it per key); the per-key rule
  (lead-scoped for lead-authority keys, non-empty for `prompt.*`, none for
  `agents.tier`) is enforced at dispatch, reproducing pre-collapse behavior. Minor
  caller-visible schema relaxation, recorded in the commit body.
- `buildTuningCatalog` now emits `Tool: "config.tune"` with a fixed `key` per knob
  (Decision 7 makes `config.tune` the sole writer; the catalog cannot reference
  removed tools); `configKeyEntryForTool` guards the two generic names.

Review: partitioned correctness/fit/test all at opus (risk-based escalation for the
atomic public-surface change). Fit + test clean first pass. Correctness found 1
critical + 1 minor, both fixed in `96604020` and confirmed `[resolved]` on
re-review:
- Critical: `config.tune`'s `agents.tier` write had added an up-front tier enum
  check that rejected the documented synonyms (`light/core/deep`,
  `haiku/sonnet/opus`) and case/whitespace variants the old tool accepted — a
  Decision 9 behavior-equivalence + CLI/MCP-parity regression. Fixed by letting
  `SetAgentsTierForHarness`/`normalizedTier` own tier validation; regression test
  `TestServeStdioConfigAgentsTierAcceptsTierSynonyms` added.
- Minor: empty pointId (bare `"prompt."`) was no longer rejected; guard restored
  with a negative test.

Verification: `go build`/`vet`/`test` green (`WS_SKILLS_ROOT` set); wsflow Python
suite green (10 tests); `tools/list` shows exactly `config.list` + `config.tune`
with the ten gone; wsflow catalog omits `prefer_mercenary` and
`config.tune("workflow.prefer_mercenary",...)` is rejected in no-agent mode;
`agents.tier` writes with explicit `harness` land in the correct per-harness alias
slot; wsflow rsrc regenerated byte-identical to canonical via the two `-count=1`
regen tests; no plugin version strings touched (the bump rides the eventual
goal→develop merge).

Spec/mental-model/manual docs (`mcp-tools.md` Config Tools section,
`workflow-skills.md` lead-tune ref, `mcp-runtime.md`, and the `lead-tune` +
`lead-workflow-manual` prose) were rewritten within the same atomic landing; anchor
ids preserved.

> Forward: `260626-bug-sage-review-config-setter-missing` is now unblocked — its
> `sage_review` writer can be re-designed as a registry entry targeting
> `config.list`/`config.tune` (its premise-invalidation note points here).
