...
### Result (6e4ae178) - 2026-09-23

- `agents.tier` accepts arbitrary effort labels with trim/lowercase normalization; `""` and `"none"` remain unset. The config catalog exposes effort as a string without an enum, and the CLI help no longer advertises a fixed vocabulary. Structured storage and resolution retain the supplied label; provider acceptance remains a launch concern.
- Shipped default and Codex aliases now seed small `gpt-6-luna/high`, medium `gpt-6-luna/max`, large `gpt-6-sol/high`, and xlarge `gpt-6-sol/max`. Existing user aliases and Pi-specific mappings remain untouched; tests pin both default literal paths and harness fallback/override precedence.
- Configuration, MCP resolution, and render tests cover `max`, a provider-specific label, empty/none, and the shipped tier table. Round-one review found that arbitrary control characters could inject line-oriented render metadata and that default tests inherited global config. Follow-up `93dbe7f5` quotes control characters only in textual presentation and isolates those tests; round-two correctness and test fix verification were clean.
- Verification: `cd agents-plugin-tool && go test ./...`, `scripts/smoke-ws-mcp.sh ..`, and `git diff --check 6ccb18de..HEAD` passed after the follow-up. No provider-launch acceptance was claimed or tested.
