...
- **Storage:** a single tracked dotfile ledger under `ai-docs/` (per-track — it
  travels with the branch tree, so different tracks never collide). Each review
  appends `<base>..<head>: verdict`. Not `AGENTS.md` (a fast-advancing SHA would
...
- **Checkpoint recompute + nudge (cheap, no review spawn).** At observed MCP
  ...
  before the ticket closes), `workflow_manual` at session start, `enter.*` as
  backstops — recompute the *size* of `marker..HEAD` on the review-track branch
...
- **Sweep (separately invoked).** The sweep is a `lead-review` **range-scenario**
  ...
  auto-triggered by a checkpoint. On a passing/settled verdict it appends the
  Phase-1 ledger entry (`<base>..<head>: verdict`), which advances the marker.
  Assign this actor explicitly: the range review runs through ②, and the
...
- **Nudge scaling metrics (both SHA/commit-based, per the epic's
  ...
  metadata, not the range key). Advisory only.
