...
Close both Explore tickets from the completed live matrix. Next reconcile `260906-bug-ws-pi-rsrc-mirror-drift`: its implementation is present, but honest validation requires completing `260911-chore-ws-pi-track-sync-to-refound` and exercising the normal released-runtime identity rather than the temporary `.local-devenv-runtime` marker.
