...
| 2026-09-12 | `260907-feat-ws-pi-persistent-explore-deep-research` | Fresh simple `explore-13` settled, accepted a follow-up through the same alias, and settled again with cited read-only evidence. Deep `explore-12` synthesized one blocking collection, was explicitly stopped during a subsequent turn, auto-resumed on reissued work, and synthesized a second collection. Lead-side registry and JSONL evidence retained parent `openai-codex/gpt-5.6-sol/medium`; both collection tool results resolved independently to `small` / `openai-codex/gpt-5.6-luna/high`, and the transcript contains exactly two blocking calls. | Pre-restart live matrix passed. Full Pi process exit/reopen plus post-restart follow-up remains. |
| 2026-09-12 | `260907-bug-ws-pi-deep-explore-missing-collection-tool` | After a fresh launch with the guard present, deep collection succeeded twice against small read-only repository questions and reached researcher synthesis. Each blocking tool result carried successful `state: done` metadata and a resolved small Luna/high profile; no launch or registration error occurred. | No deep-group defect reproduced. Confirm the same surface once after full process reopen, then close Phase 2 as a no-fix live verification result. |
...
Checkpoint is immediately before the mandatory process boundary. Fully exit Pi, reopen this exact lead session, restore ws workflow state with session key `shiny-chokehold-tribesman`, confirm `explore-12` restores as the same dormant deep record with frozen parent model/effort, then send it one final follow-up that performs a small blocking collection and verify the result metadata in the restored transcript.
