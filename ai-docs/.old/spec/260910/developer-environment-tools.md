...
It also reports when output tokens last changed, as an absolute clock time. That
timestamp is read from the transcript file the status JSON points at rather than
from persisted state, so it degrades to a placeholder when the transcript is
unreadable instead of failing the render.
