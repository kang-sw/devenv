...
- `install.sh update` refreshes the installed `agents-plugin/` snapshot and marketplace metadata; it must not auto-install wsflow for Claude. {#260505-claude-install-update-snapshot} {#260513-wsflow-installer-snapshot}
...
- **Change Claude local installer registration**: update `install.sh` so the marketplace snapshot, enabled plugin list, stale installed-plugin repair, and manual fallback messages stay aligned for the ws plugin; do not add wsflow auto-install without explicit user approval. {#260513-wsflow-installer-snapshot}
