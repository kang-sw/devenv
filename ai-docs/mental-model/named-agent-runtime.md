...
- `ToolProfile` flows into subprocess env as `WS_MCP_TOOL_PROFILE` when the host preserves it; MCP treats it as an optional profile filter, not an authority boundary. Verified non-functional as a containment barrier across several checks (2026-06): the env profile does not actually restrict the spawned child's exposed tool surface, so it cannot deny capabilities in a spawned subprocess. Treat it as an advisory hint only. The session-key keyed capability gate (see `mcp-runtime`) is the first server-side containment hook, but Phase 2a spawns do not inject delegated child credentials; mercenary child-key minting is Phase 2c and full capability-scope enforcement is Phase 3.
...
- MCP `agents.*` lifecycle tools require the caller's mandatory `session_key`; `mcp.resolveToolRoot` converts that key to the worktree root passed into `wsagent.Manager`. There is no actor binding, hidden explicit-root namespace, or actor-scoped dispatch path in Phase 2a. {#260610-ephemeral-session-auth-model}
- Phase 2a spawns compile without injected child credentials: `ensureChildActor`, `childActorInstruction`, persistent child actor ids, actor-scoped registration/dispatch/inbox, and child-inactivation on ephemeral result consumption are removed. Phase 2c owns render-minted child keys for mercenary delegation; until then retained Codex/Claude runner behavior is otherwise unchanged by the auth cutover. {#260610-mercenary-delegation-surface}
