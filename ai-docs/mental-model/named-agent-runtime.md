...
- MCP `agents.register` schema accepts only `name`, `backend`, and `system_prompt_text` (Phase 2c); `model`, `tier`, `prompts`, and `prompt_refs` are removed from the MCP dispatch layer. Pass the rendered output of `playbook.render` as `system_prompt_text`; per-mercenary model selection moves to the rendered prompt body and harness config (`#260513-harness-local-agent-tier-config`). Internal callers that invoke `wsagent.Register` directly (api_docs, oneShot) still have access to `RegisterOptions.Prompts`/`Model`/`Tier`. {#260610-mercenary-delegation-surface}
...
- `ToolProfile` flows into subprocess env as `WS_MCP_TOOL_PROFILE` when the host preserves it; MCP treats it as an optional profile filter, not an authority boundary. Verified non-functional as a containment barrier across several checks (2026-06): the env profile does not actually restrict the spawned child's exposed tool surface, so it cannot deny capabilities in a spawned subprocess. Treat it as an advisory hint only. The session-key keyed capability gate (see `mcp-runtime`) is the first server-side containment hook; render-minted child keys (Phase 2c, implemented) inject scoped delegate credentials, but full capability-scope enforcement (WS_MCP_TOOL_PROFILE fold) is Phase 3.
