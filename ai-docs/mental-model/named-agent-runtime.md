...
- MCP `ws.mercenary.register` schema accepts `name`, `backend`, `system_prompt_text`, and `tier`; `model`, `prompts`, and `prompt_refs` stay removed from the MCP dispatch layer (Phase 2c). Pass the rendered output of `playbook.render` as `system_prompt_text`. `tier` was re-introduced in Phase 2 (260611) as a pass-through of `playbook.render`'s `recommended-tier` (origin = playbook frontmatter, mapped first-class→alias by the handler), so a mercenary's per-call model follows its playbook frontmatter tier via harness config (`#260513-harness-local-agent-tier-config`) instead of defaulting to core. Internal callers that invoke `wsagent.Register` directly (api_docs) still have access to `RegisterOptions.Prompts`/`Model`; the test-only `oneShot` helper was removed in Phase 2. {#260610-mercenary-delegation-surface}
