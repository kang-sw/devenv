...
related:
  ...
  workflow-skills: "skills render rsrc delegate playbooks via playbook.render for delegation; embedded stems remain only for internal callers (api_docs)."
...
- Shipped skills name rsrc delegate playbooks (resolved by `wsrsrc`, manifest-checked); API docs code still names embedded prompt stems directly. Renaming either requires updating those callers.
- The rsrc delegate playbooks are the canonical skill delegation surface: full-ws workflow skills render them via `ws/playbook.print`/`ws/playbook.render` to hand a worker a self-contained prompt (Phase 5, 260611, migrated every delegate off `register(prompts:[stems])`). `explore` (`agents-plugin/rsrc/explore/explore.md`, `kind:render`, `delegates:true`) was the precedent and remains the scoped-fact-finding delegate; renaming its declared variables (`ExploreAgent`, `SpawnIdiom`, `ContinueIdiom`), removing `delegates:true`, or restructuring its body breaks skill delegation across all call sites without a code-layer compile error. The `implementer`/`reviewer`/`reference-discovery`/`mental-model-updater`/`plan-populator-*` playbooks are likewise rendered, not registered by stem.
...
- The three `code-review-{correctness,fit,test}` partition playbooks share the reviewer base through `includes: [code-reviewer]` — a root-level flat dep (`agents-plugin/rsrc/code-reviewer.md`) with no frontmatter and no model-var placeholder. Editing that file changes all three partitions; it must stay var-free so partitions with different tiers (`large` vs `medium`) do not collide on an undeclared model var. A subdir playbook (e.g. the Phase 1 `reviewer/`) cannot be an include target (includes resolve flat to `<root>/<name>.md`), so the base reviewer text is intentionally duplicated between `code-reviewer.md` and `reviewer/reviewer.md` until a later phase (260611 Phase 5/6) reconciles the call sites.
