...
- Release repair requires matching binary asset names and `SHA256SUMS`; local dev repair is gated to the local installed plugin cache, `.local-devenv-runtime`, and non-Windows platforms. Bootstrap binary/URL inputs and local devenv repair intentionally force install before compatibility-stamp reuse so plugin-cache dogfood can exercise a fresh local runtime; the forced local path builds from source before considering local dist assets. {#260505-release-asset-build-checksum-pipeline}
