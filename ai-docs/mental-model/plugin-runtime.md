...
- `ai-docs/manuals/ws-mcp.md` is a launcher and operations runbook, not the plugin runtime contract; update `spec/plugin-runtime.md`, `runtime.json`, and `runtime.capabilities` when caller-visible compatibility behavior changes. {#260524-reference-document-ownership}
...
- Assigning `ws-mcp serve` to a Windows Job Object with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` without a companion breakaway: on Windows a child inherits its parent's job unless the job sets `JOB_OBJECT_LIMIT_BREAKAWAY_OK` **and** the child is spawned with `CREATE_BREAKAWAY_FROM_JOB`, so a naive kill-on-close job would cascade-kill mercenary async workers and break the survives-disconnect contract. Windows orphan reaping is instead done server-side (`cmd/ws-mcp/parent_watch_windows.go` self-terminates on parent death, which does not touch independent child processes); the launcher-side Job Object backstop is deliberately deferred until real-Windows evidence shows server-side detection is insufficient. The kill-on-close mechanism also could not be validated through WSL→`powershell.exe` interop (assignment verified via `IsProcessInJob`, yet the child was not reaped), confirming it needs a real Windows session/CI, not this dev box. {#260724-windows-process-lifecycle-hardening}
