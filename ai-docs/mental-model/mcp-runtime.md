...
- Root-omitted `agents.*` lifecycle tools and `subquery` resolve through the current actor scope when the resolved root is actor-bound; this includes legacy `agents.print`. Hidden explicit-root arguments intentionally bypass actor scope and use the global compatibility namespace. {#260524-mcp-actor-setup-bootstrap}
