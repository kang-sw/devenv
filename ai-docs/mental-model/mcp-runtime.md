...
- Keyed capability gate in `callTool`: when a non-empty `session_key` maps to a known non-lead scope, `roleAllowsTool(scope, toolName)` is checked and any `ws.lead.*` prefix is additionally blocked. Unknown `session_key` values are NOT rejected here; root-aware tools surface `unknown_session` through `resolveToolRoot`. Phase 2a still mostly mints lead keys; mercenary child-key minting is Phase 2c, and full capability-scope enforcement remains Phase 3.
...
- Treating `WS_MCP_TOOL_PROFILE` or schema-level filtering as an authority boundary creates false safety; they remain advisory. Mandatory `session_key` closes keyless root-aware dispatch, but capability-scope enforcement is still Phase 3 and prompt-level delegate rules remain necessary until scoped keys are actually minted/enforced end-to-end.
