...
- `ws.lead.prefer_mercenary` is a lead-only tool that flips the `preferMercenary` flag in the caller's session record via an atomic read-modify-write (one-way; cannot be unset in the current session). After the flip, `playbook.render` injects a mercenary-primary guidance block into implementer and reviewer playbook renderings; it does not affect tool availability — the mercenary path is always reachable via the always-on tip appended to every `delegates:true` rendering. Hidden in wsflow no-agent mode (`noAgentHiddenTool`); `ws.ferrule` stays visible. {#260610-mercenary-delegation-surface}
- `sessionStore` is a flat filesystem-backed store: one JSON record per key at `<cache-root>/keys/<key>.json` (cache root honors `WS_CACHE_HOME`), no SQLite and no single shared file. The file is the source of truth, so a fresh server instance or a restarted lead resolves keys by reading them; state survives a server restart. There is still no logout and no automatic eviction, but deleting a key file is now a physically possible removal path. Mint claims a unique key with `O_EXCL` create; `setPreferMercenary` updates via temp-write + rename; lookup rejects path-unsafe keys before touching the filesystem. Session keys are opaque word-chain strings; do not parse them.
