...
- `ws.lead.login` mints an ephemeral word-chain session key associating a canonical repository root and a capability scope (`lead`/`delegate`/`leaf`; absent → `lead`) in the session store. It validates `root` via `canonicalSetupRoot` (catches `"<cwd>"` placeholders). A non-lead scoped key cannot call any `ws.lead.*` tool (self-login escalation block): the prefix is checked server-side before dispatch, not at the schema level.
