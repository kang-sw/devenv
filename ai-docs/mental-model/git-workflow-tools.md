...
- The ticket-verify gate (`#260723-git-commit-ticket-verify-gate`) does not receive the raw expanded path list: index-delete-side paths are filtered out first — index status `D` drops `Path`, `R` drops `OldPath`, `C` (copy) is kept — and the verifier call is skipped entirely when nothing survives the filter. `validateCommitStatus`'s unrelated-staged-path check is unaffected and still sees the full expanded list. This is what makes a `tickets.move`/`tickets.close` status transition, and an outright staged ticket deletion, committable. When the verifier is skipped this way, `CommitResult.Advisories` stays empty too — there is no verifier call to produce any. {#260725-git-commit-verify-excludes-delete-side-paths}
- `wsgit.Verifier` is `func(root, paths) ([]string, error)`: the slice carries pre-formatted, order-preserving advisory text lines that `Client.Commit` threads into `CommitResult.Advisories` without affecting the commit outcome; the error is still the veto. A non-nil error stops `Client.Commit` before the commit lands, and the advisories returned alongside that error are discarded — a veto has nothing to advise on (pinned by `TestCommitVetoDiscardsAdvisoriesAlongsideError`). `verifyAdapter` (internal/mcp, not `wsgit`) is the sole producer today: on the pass branch it formats `wsdoc.VerifyResult.Warnings` into `WARN [guardrail] path: message` lines — the same shape `formatTicketVerify` uses — keeping warning-to-text formatting out of `wsgit` per the `{#260720-wsdoc-commit-boundary}` import boundary. `CommitResult.Advisories` carries `json:"-"` (text-mode only, following the todo-reinjection precedent below), so both JSON emission sites omit it with no per-call-site branch; see `{#260727-git-commit-verify-advisories}` for the caller-visible response shape.
- `verifyAdapter` also folds `wsdoc.VerifyResult.Advisories` (the cross-file ticket-graph pass owned by wsdoc, `{#260727-tickets-verify-graph-advisories}`) into the same `[]string` channel, verbatim, and appends the `Then git commit --amend --no-edit.` recipe only to entries whose `Kind` is `wsdoc.AdvisoryKindFix` — keyed on the typed field, never on the rendered `FIX:`/`CHECK:` prefix, so the settled output text is never load-bearing for that decision. `formatGitCommit` separates every advisory (not just the first) with a blank line: Phase 2 puts a multi-line `## Parent Board` block on the same channel as single-line warnings, and without the separator they render as one run-together wall of text.
...
- `git.commit`'s text-mode response builds trailers in a fixed order: commit output (with an `advisories:` block appended inline by `formatGitCommit` itself when `CommitResult.Advisories` is non-empty — each advisory is split on `\n` and every resulting line is indented, not just the first, so a future multi-line advisory block does not lose its indentation past line 1), then the todo re-injection block (if the session holds todos), then `appendSessionKeyTip`'s `tip: preserve this session key: <key> during compaction` line (if a session key is present). The session-key tip must stay last so it is the most recent line before compaction; when adding another text-mode trailer to `git.commit`, insert it before the tip call in `server.go`, not after. {#260708-git-commit-session-key-tip}
