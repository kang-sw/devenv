...
- `ws-new-agent` / `ws-call-agent` named registry: agent slots must be registered via `ws-new-agent <name> --model <level> --system-prompt "$(ws-infra-path <doc>)"` before any `ws-call-agent` call for that name. `ws-new-agent` stores model, agent type, and system prompt in `.git/ws@<repo-dir>/agents/<name>.json` and issues a fresh session UUID. `ws-call-agent <name> <prompt>` reads all config from the registry — model, system prompt, session routing — with no per-call flags. Calling `ws-call-agent` before `ws-new-agent` exits 1 with "agent not found". `ws-declare-agent` is now only for cleaning orphaned session files from prior runs without re-registering.
- `ws-call-agent` auto-compression: when `token_count > 100K` tokens, `ws-call-agent` transparently replaces the active session with a fresh one. The 3-step flow is: extract intent via haiku, compress session state via the model's own context, hand off to a new agent session with compressed context prepended. The lead observes no behavioral difference — the handoff result appears on stdout as normal. Re-compression is suppressed on the immediate next call via a `compressed_at` flag in the registry.
...
- `ws:sprint` Delegation Cycle uses two reviewers only (`reviewer-correctness` and `reviewer-fit`). There is no `reviewer-test` partition in sprint sessions; the Delegation Cycle registers only two agents via `ws-new-agent`. The review-path allocation call is `review-path correctness fit` (two paths, not three). The cleanup step removes the two allocated paths after each task.
...
- Allocating three review paths (`review-path correctness fit test`) in a sprint Delegation Cycle — sprint uses two reviewers only; there is no `reviewer-test` agent registered via `ws-new-agent` in the sprint session.
