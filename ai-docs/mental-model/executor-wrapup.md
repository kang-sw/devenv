...
- `ws-call-named-agent` auto-compression: when `token_count > 120K` tokens, `ws-call-named-agent` transparently replaces the active session with a fresh one. The 3-step flow is: extract intent via haiku, compress session state via the model's own context, hand off to a new agent session with compressed context prepended. The lead observes no behavioral difference — the handoff result appears on stdout as normal. Re-compression is suppressed on the immediate next call via a `compressed_at` flag in the registry. Token count is backend-dependent: the claude backend sums `input_tokens + cache_creation_input_tokens + output_tokens`; the codex backend sums `input_tokens + output_tokens` only (no cache field). The registry records the computed value in both `token_count` and `context-window`.
...
- **Add a new executor skill**: decide which pattern to follow:
  ...
  - Sprint-like (session container, feature branch): suppress per-task doc pipeline; at session end dispatch `ws:spec-updater` in Suggestion mode, apply proposed strips, then run `ws:mental-model-updater`, then call executor-wrapup, then auto-merge.
