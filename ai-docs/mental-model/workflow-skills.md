...
- wsflow `lead-implement` is the converged unified spine (no separate `lead-edit`): Edit is lead-owned direct edits plus optional scoped native subagents with no implementer stage, and survey/plan/review/mental-model delegate prompts dispatch through `wsflow/prompt.render` to native subagents rather than ws named agents. {#260529-wsflow-converged-implement-spine}
