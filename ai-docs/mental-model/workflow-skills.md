...
- `lead-proceed` treats `todo/` ticket paths as implementation intent, routes them through `lead-write-ticket` for ready promotion rather than pre-routing through `lead-write-spec`, and escalates only for unresolved design or spec-address/readiness blockers. {#260505-proceed-routing-pipeline}
