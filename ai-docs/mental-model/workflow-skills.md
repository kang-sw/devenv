...
- Skills delegate scoped codebase fact-finding via the native Explore pattern: render the `explore` rsrc playbook via `ws/playbook.render(name: "explore", context: {...})` or `ws/playbook.print`, spawn a native subagent with the rendered brief, collect the deferred result; for parallel surveys, spawn multiple subagents in one turn and collect all before synthesizing. `ws/subquery` is removed; the native Explore subagent is the exclusive delegation path. {#260610-subquery-explore-delegation-shift} {#260609-rsrc-playbook-distribution}
