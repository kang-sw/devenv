...
- Dense skill routing should use Markdown hierarchy, grouped invariants, fixed lookup tables, shared handler-label vocabulary, and command-shaped lists before custom notation; skill, agent, and prompt edits run a fresh-reader audit, and doctrine/terminology/routing/layout edits run a downstream consistency sweep before accepted fixes land. {#260514-skill-authoring-carried-context}
