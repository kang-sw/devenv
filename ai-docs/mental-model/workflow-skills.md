...
- `lead-proceed` routes implementation-ready work to `lead-implement`; it resolves implementation scope and source-free dispatch lower bounds, but does not decide contract-brief depth, inspect source, or invoke implementation primitives directly. {#260505-proceed-routing-pipeline} {#260519-proceed-implementation-dispatch-precheck}
...
- `lead-implement` may escalate a proceed-selected direct edit to `lead-write-code`, but must not downgrade a proceed-selected `write-code` lower bound. {#260519-proceed-implementation-dispatch-precheck}
...
- wsflow mirrors proceed dispatch clarity as execution path, complexity flag, and branch mode; it does not mirror full ws implementation relay names for excluded skills. {#260519-proceed-implementation-dispatch-precheck}
