...
- Broad `specs.query` and `mental_models.query` query matching is token-scored in shared wsdoc helpers, not per-discovery substring checks; exact selectors (`spec_stem`, `ticket_stem`, `domain`) still filter before query scoring. {#260519-tolerant-documentation-lookup-query-evidence}
...
- The same asymmetry rule applies to a *fix*, not only to output, and this is where it is least visible: replacing a recoverable source with a better but ephemeral one is a narrowing whenever the fix also prescribes against the old source. `lead-forge-spec`'s wrap-up moved its ambiguity list from the on-disk `<!-- AMBIGUOUS -->` markers to an in-session record — correct, since items classified planned or internal-only are never written to a spec and so carry no marker — and added "not from the markers"; a resumed session holds no record, so the one reconstruction path was now forbidden and the report emitted a confident zero instead of an incomplete list. The pre-image was also non-durable, which is why this reads as no regression until the two branches are ranked by destructiveness: "undescribed recovery" became "prescribed-against recovery". Rule: when the better source is session-scoped, keep the durable one as a named fallback at the consumer and close on a labelled partial (`reconstructed from X; items of class Y are not recoverable`) rather than a silent complete. Both the original hole and its repair were introduced by fixes inside one ticket, one axis apart — so a fix that narrows what a step collects must be re-checked against the collection's downstream promise one layer out, including across a resume boundary.
...
- Moving current feature inventory or implementation status into the project reading map instead of specs, tickets, source, or tests (that inventory is generated now, not hand-kept in a project-memory file).
