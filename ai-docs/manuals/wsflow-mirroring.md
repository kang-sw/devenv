...
- Exception: `lead-prefer-subagent` ships an inline procedure body rather than
  a `playbook.read` shim, generated by the substitution-mirrored skill
  mechanism below rather than hand-authored. Its full-ws counterpart is
  likewise an inline `agents-plugin/skills/lead-prefer-subagent/SKILL.md` body,
  not a `playbook.read`-backed rsrc playbook — the shared rsrc playbook dir for
  it was deleted. It is also in `EXPECTED_INLINE_SKILLS` for the same reason as
  `lead-revive`.
