...
- Exception: `lead-prefer-subagent` and `lead-verify-discussion` ship inline
  procedure bodies rather than `playbook.read` shims, generated by the
  substitution-mirrored skill mechanism below rather than hand-authored. Their
  full-ws counterparts are likewise inline
  `agents-plugin/skills/lead-*/SKILL.md` bodies, not `playbook.read`-backed
  rsrc playbooks — the shared rsrc playbook dirs for these were deleted. Both
  are also in `EXPECTED_INLINE_SKILLS` for the same reason as `lead-revive`.
