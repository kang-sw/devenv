...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.46.5`. |
...
plus both `ws-pi-bridge` `package.json` versions). Development binaries such as `0.46.5-dev` satisfy the
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.46.5-dev
