...
| `WS_MCP_RELEASE_TAG` | Override the release tag from `runtime.json`, for example `v0.46.17`. |
...
plus both `ws-pi-bridge` `package.json` versions). Development binaries such as `0.46.17-dev` satisfy the
...
because no bump-script regex matches a bare `X.Y.Z`.
...
```bash
cd agents-plugin-tool
scripts/build-release-assets.sh 0.46.17-dev
