...
their delegate prompts through `prompt.render` or, during the convergence
migration, through wsflow-mode `playbook.render` for the same five legacy stems
(see `#260529-prompt-render-tool`): the lead renders the chosen prompt to a
path, hands it to a native subagent, and integrates the subagent's returned
result.
