...
- **Downstream portability** — bare `claude/infra/` paths, renamed tools, hardcoded CWD assumptions (`ws:code-reviewer`). A known-renamed-tools list is embedded in the reviewer prompt to catch stale references by name.

**Scope:** `claude/skills/`, `claude/agents/`, and `claude/bin/`.
