...
of canonical `agents-plugin/rsrc/`, so the wsflow-only `prompt.render` tool
(`#260529-prompt-render-tool`) resolves its prompts from rsrc rather than the
embedded bundle. The `ws/` -> `wsflow/` substitution is render-time, so the
stored copy does not diverge from canonical; a byte-equality drift guard
