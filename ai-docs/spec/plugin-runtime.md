...
wsflow-only marker blocks and applies `ws` -> `wsflow` namespace substitution,
while the older wsflow-only `prompt.render` tool still applies its own
allowlisted prompt substitution until its migration phase removes it. The stored
rsrc copy does not diverge from canonical; a byte-equality drift guard
