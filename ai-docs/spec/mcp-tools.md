...
wsflow product mode. It loads a delegate prompt by `stem` from the rsrc tree
(a generated byte-identical copy of canonical `agents-plugin/rsrc/` shipped with
the wsflow package; see `#260513-wsflow-agentless-plugin-package`), applies the
render-time `ws/` -> `wsflow/` namespace substitution used for wsflow-facing
text, appends caller-supplied `context` values as a free-text Render Context
block after substitution, writes the rendered result to a temporary file, and
returns that file path. The caller hands `prompt_path` to a native host subagent.
The backing source is the rsrc loader (the embedded prompt bundle is no longer
the source); the render-eligibility allowlist and the namespace-substitution
