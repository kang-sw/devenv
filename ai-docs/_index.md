...
Active plugin package: `agents-plugin/` (`ws@0.27.0`).
Agentless derivative package: `agents-plugin-wsflow/` (`wsflow@0.27.0`).
...
| `260523-bug-agents-root-schema-visible` | ready | Remove visible `root` parameters from public/generated `agents.*` schemas while preserving `ws.setup` as the session-root surface |
...
- `260523-bug-agents-root-schema-visible` - Hide `root` from public/generated `agents.*` schemas while preserving hidden compatibility override behavior and `ws.setup` as the normal session-root surface.
