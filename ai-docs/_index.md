...
| `260727-bug-dashboard-terminal-refocus-ime-composition-loss` | ready | Terminal refocus drops an in-flight IME composition; guard fix landed, ticket still open |
...
| `260729-bug-dashboard-agent-callback-url-uses-bound-interface` | ready | Agent callback base URL is built from the bound interface, so a public bind POSTs the per-terminal bearer token in cleartext and `0.0.0.0` breaks the hook on Windows; fix is a callback-only loopback listener, bound only when the primary bind is an explicit non-loopback address |
| `260729-bug-dashboard-agent-profile-gc-destroys-concurrent-daemon-state` | ready | A busy helper is indistinguishable from a dead one (single-connection accept loop + discarded connect error), so `boot_reconcile`, the 10s sweep and the profile GC destroy live terminals owned by another daemon; fix is a concurrent liveness probe with a three-way predicate, not state-dir partitioning |
...
| `260729-bug-dashboard-macos-terminal-socket-path-and-eperm-gaps` | ready | macOS terminal helper: the `sun_path` ceiling (103 usable, not 104) is unguarded in production, so a deep state home fails every terminal with a misattributed handshake diagnostic; Phase 2 corrects the EPERM CONTRACT and a restore-model spec sentence, doc-only |
