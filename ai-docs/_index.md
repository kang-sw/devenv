...
| `260729-bug-dashboard-agent-callback-url-uses-bound-interface` | todo | Agent callback base URL is built from the bound interface, so a public bind POSTs the per-terminal bearer token in cleartext and `0.0.0.0` breaks the hook on Windows. Sage-blocked: Phase 1 needs a decision between conditional loopback / declared degradation / a second loopback listener |
| `260729-bug-dashboard-agent-profile-gc-destroys-concurrent-daemon-state` | todo | Two daemons on one state dir SIGKILL each other's live terminals every 10s via `sweep_registry_backstop`; the profile GC then leaves the state unrecoverable, and the escalation meant to surface it is structurally silent. Sage-blocked: needs a product decision on whether a shared state dir is supported |
