...
Active plugin package: `agents-plugin/` (`ws@0.38.1`).
Agentless derivative package: `agents-plugin-wsflow/` (`wsflow@0.38.1`).
...
| Stem | Status | Summary |
|------|--------|---------|
| `260806-feat-worktree-ticket-scope` | ready | Per-worktree ticket scope via sparse-checkout with index-aware board resolution (both phases implemented; `ws:lead-scope-worktree` skill + `workflow_manual` scope gate landed, merge pending) |
