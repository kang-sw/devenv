...
export async function runClaudeItem(input: ClaudeRunInput, dependencies: ClaudeSdkDependencies = {}): Promise<ClaudeRunOutput> {
  ...
  try {
    ...
    query = sdk.query({ prompt: buildClaudeRequest(input), options: buildClaudeOptions(input, resolveClaudeExecutable(dependencies.executable), dependencies.env, (options) => { if (finalized || input.abortController.signal.aborted) throw new ClaudeDelegateError("cancelled", SAFE.cancelled); child = spawnOwned(options); return child; }) });
  ...
  finally { finalized = true; if (!(await disposeClaudeChild(query, child, dependencies.cleanupMs))) throw new ClaudeDelegateError("cleanup_failed", SAFE.cleanup_failed); }
}
