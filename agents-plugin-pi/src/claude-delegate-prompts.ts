export type ClaudeDelegatePreset = "audit" | "consult" | "rewrite" | "design-review";
...
export function buildClaudeTaskFrame(preset: ClaudeDelegatePreset): string {
  ...
  if (preset === "rewrite") return "You are helping with a focused rewrite. Before editing, read the target artifact, AGENTS.md, and the applicable project conventions or manuals they identify, to prevent the rewrite prompt from drifting from the project's actual conventions. Improve only the explicitly authorized edit targets, preserve the requested intent, and finish with a concise change summary. If required context is unavailable, say what is missing instead of editing by assumption.";
  return "You are reviewing ticket design using the canonical reviewer playbook supplied by the parent adapter. Use only its read-only context bundle, preserve its criteria and verdict format, and report missing context instead of inventing a complete review.";
}
