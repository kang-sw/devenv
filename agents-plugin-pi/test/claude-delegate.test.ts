...
function fakeSdk(result = "ok") {
  return { query: () => ({
    close() {},
    async *[Symbol.asyncIterator]() { yield { type: "result", subtype: "success", is_error: false, result, session_id: "fixture-session", modelUsage: { input_tokens: 1 } }; },
  }) };
}
...
test("ws-claude returns index-aligned results and preserves invalid siblings", async () => {
  ...
  const results = await controller.execute([{ preset: "audit", request: "check this" }, { preset: "unsupported", request: "no" }]);
...
test("shared controller admits only three items and rejects edit targets for read-only presets", async () => {
  ...
  const controller = createClaudeDelegateController(() => process.cwd(), { executable: "/usr/bin/true", loadSdk: async () => ({ query: () => ({ close() {}, async *[Symbol.asyncIterator]() { active += 1; maximum = Math.max(maximum, active); await new Promise<void>((resolve) => releases.push(resolve)); active -= 1; yield { type: "result", subtype: "success", is_error: false, result: "ok", session_id: "fixture-session", usage: {}, modelUsage: {}, total_cost_usd: 0 }; } }) }) });
  ...
  try {
    ...
    const controller = createClaudeDelegateController(() => root, { executable: process.execPath, loadSdk: async () => ({ query: ({ prompt, options }: any) => ({
      close() {}, async *[Symbol.asyncIterator]() {
        ...
        assert.equal(permission.behavior, "allow"); writeFileSync(target, `new-${first ? "first" : "second"}`); yield { type: "result", subtype: "success", is_error: false, result: "rewritten", session_id: "fixture-session" };
      },
    }) }) as any });
...
test("an authorized new file is reported changed and remains untracked", async () => {
  ...
  try {
    ...
    const controller = createClaudeDelegateController(() => root, { executable: process.execPath, loadSdk: async () => ({ query: ({ options }: any) => ({ close() {}, async *[Symbol.asyncIterator]() {
      ...
      assert.equal(permission.behavior, "allow"); writeFileSync(target, "new"); yield { type: "result", subtype: "success", is_error: false, result: "created", session_id: "fixture-session" };
    } }) }) as any });
...
test("overlapping canonical rewrite targets are rejected while a sibling completes", async () => {
  ...
  try {
    ...
    const controller = createClaudeDelegateController(() => root, { executable: process.execPath, loadSdk: async () => ({ query: () => ({ close() {}, async *[Symbol.asyncIterator]() { queries++; await gate; yield { type: "result", subtype: "success", is_error: false, result: "ok", session_id: "fixture-session" }; } }) }) as any });
    const pending = controller.execute([
...
test("actual registration exposes the object envelope and starts no child before execution", async () => {
  ...
  ref.current = createClaudeDelegateController(() => process.cwd(), { executable: "/usr/bin/true", loadSdk: async () => ({ query: () => { queries += 1; return { close() {}, async *[Symbol.asyncIterator]() { yield { type: "result", subtype: "success", is_error: false, result: "ok", session_id: "fixture-session", usage: {}, modelUsage: {}, total_cost_usd: 0 }; } }; } }) });
...
test("queued invocation cancellation settles queued work without launching it", async () => {
  let starts = 0; const gate: (() => void)[] = []; const controller = createClaudeDelegateController(() => process.cwd(), { executable: "/usr/bin/true", loadSdk: async () => ({ query: () => ({ close() {}, async *[Symbol.asyncIterator]() { starts += 1; await new Promise<void>((resolve) => gate.push(resolve)); yield { type: "result", subtype: "success", is_error: false, result: "ok", session_id: "fixture-session", usage: {}, modelUsage: {}, total_cost_usd: 0 }; } }) }) });
