...
import { buildClaudeOptions, createClaudeEditScope, delegateEnvironment, runClaudeItem } from "../src/claude-sdk.ts";
...
test("rewrite permission revalidates exact canonical targets and rejects path escapes", async () => {
  ...
  try {
    writeFileSync(join(root, "allowed.txt"), "old"); writeFileSync(join(root, "mutable-source.txt"), "mutable"); writeFileSync(join(outside, "outside.txt"), "outside"); mkdirSync(join(root, "parent"));
    ...
    for (const denied of [join(root, "..", "outside.txt"), join(outside, "outside.txt"), join(root, "escape.txt"), join(root, "escape-parent", "new.txt")]) assert.equal((await decide(denied)).behavior, "deny");
...
test("rewrite targets reject traversal and nonexistent or escaped parents before SDK launch", () => {
  ...
  try {
    ...
    for (const targets of [["../escape.txt"], [join(outside, "escape.txt")], ["missing-parent/new.txt"], ["alias/new.txt"]]) assert.throws(() => createClaudeEditScope(root, targets), /edit target/);
  } finally { rmSync(root, { recursive: true, force: true }); rmSync(outside, { recursive: true, force: true }); }
});
