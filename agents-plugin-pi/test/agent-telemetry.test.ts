...
test("telemetry reducer excludes the immutable prefix and recomputes replay without double counting", () => withSession([header, assistant("parent", 9, 9), assistant("a", 10, .1), assistant("b", 25, .1)], path => {
  ...
  assert.deepEqual(reduceTelemetry({ sessionId: "child", sessionPath: path, prefixEntryId: "parent" }, read), { contextTokens: 25, estimatedUsd: .2 });
  assert.deepEqual(reduceTelemetry({ sessionId: "child", sessionPath: path, prefixEntryId: "parent" }, read), { contextTokens: 25, estimatedUsd: .2 });
}));
...
test("stop performs a final disk reconciliation after synchronous live-state clear", async () => withSession([{ ...header, id: "stop" }], async path => {
  ...
  assert.equal(record.telemetry?.contextTokens, 11);
  assert.equal(record.telemetry?.estimatedUsd, .6);
}));

test("row projection preserves reported zero context occupancy for live and thread-only recovery", () => {
  const live = { agentId: "a", client: {}, sessionPath: "/tmp/a", wsToolNames: [], toolGroup: "full-worker", reportLog: [], streaming: true, running: true, telemetry: { version: 1, origin: { sessionId: "a", sessionPath: "/tmp/a", emptyPrefix: true }, contextTokens: 0 } } as unknown as RpcAgentRecord;
  assert.equal(buildAgentRows(new Map([["a", live]]), [], Date.now())[0].contextTokens, 0);
  const thread = { threadId: "q1", title: "q", status: "pending", origin: "fork-raised", createdAt: "2026-01-01T00:00:00.000Z", touchedAt: "2026-01-01T00:00:00.000Z", forkResume: { sessionPath: "/tmp/f", wsToolNames: [], toolGroup: "full-worker", telemetry: { version: 1, origin: { sessionId: "f", sessionPath: "/tmp/f", emptyPrefix: true }, contextTokens: 0 } } } as never;
  assert.equal(buildAgentRows(new Map(), [thread], Date.now())[0].contextTokens, 0);
});
...
test("telemetry retains reported zero and partial known cost while refusing malformed/missing anchors", () => withSession([header, assistant("a", 0, 0)], path => {
  const read = readSessionEntries(path);
  assert.deepEqual(reduceTelemetry({ sessionId: "child", sessionPath: path, emptyPrefix: true }, read), { contextTokens: 0, estimatedUsd: 0 });
  ...
  assert.deepEqual(reduceTelemetry({ sessionId: "child", sessionPath: path, emptyPrefix: true }, readSessionEntries(path)), { contextTokens: 5, partialEstimatedUsd: .25 });
...
test("binds the no-parent baseline before prompting and keeps a legacy fork floor separate from cost", () => withSession([{ ...header, id: "worker" }], path => {
  ...
  assert.equal(worker.telemetry?.contextTokens, 7);
  ...
  assert.equal(fork.observedContextTokens, 8);
  assert.equal(fork.telemetry, undefined);
}));
