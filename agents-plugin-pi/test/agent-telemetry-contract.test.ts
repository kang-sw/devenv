...
const assistantUsage = (id: string, usage: Record<string, unknown>) => ({ type: "message", id, parentId: null, timestamp: "same", message: { role: "assistant", content: "same", usage } });
...
test("durable SDK-style IDs, rather than timestamps/content/response IDs, distinguish identical calls", () => withSession([
  ...
  assert.deepEqual(reduceTelemetry(origin(path), read), { contextTokens: 10, estimatedUsd: .375 });
}));
...
test("identical replayed entry IDs contribute once, but conflicting duplicate IDs invalidate attribution", () => withSession([
  ...
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 12, estimatedUsd: .4 });
...
test("the prefix anchor excludes every earlier entry in file order, including off-branch inherited calls", () => withSession([
  ...
  assert.deepEqual(reduceTelemetry(origin(path, { prefixEntryId: "anchor" }), readSessionEntries(path)), { contextTokens: 25, estimatedUsd: .3 });
}));

test("context fallback uses totalTokens before the complete nonnegative usage-field sum", () => withSession([
  header(),
  assistantUsage("explicit-total", { input: 342, output: 7, cacheRead: 61_440, cacheWrite: 5, totalTokens: 61_900, cost: { total: .1 } }),
], path => {
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 61_900, estimatedUsd: .1 });
  writeFileSync(path, `${[header(), assistantUsage("observed-a", { input: 342, cacheRead: 61_440, cost: { total: .1 } })].map(JSON.stringify).join("\n")}\n`);
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 61_782, estimatedUsd: .1 });
  writeFileSync(path, `${[header(), assistantUsage("observed-b", { input: 3_150, cacheRead: 68_608, cost: { total: .2 } })].map(JSON.stringify).join("\n")}\n`);
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 71_758, estimatedUsd: .2 });
}));

test("context fallback sums only present nonnegative fields and never uses tool or summary usage", () => withSession([
  header(), assistantUsage("partial-provider", { input: 20, output: 3, cacheRead: -1, cacheWrite: "bad", cost: { total: .1 } }),
  { type: "message", id: "cached-tool", parentId: null, timestamp: "same", message: { role: "toolResult", usage: { input: 1234, cacheRead: 1200, cost: { total: .2 } } } },
], path => {
  const partial = reduceTelemetry(origin(path), readSessionEntries(path));
  assert.equal(partial?.contextTokens, 23);
  assert.ok(Math.abs((partial?.estimatedUsd ?? 0) - .3) < Number.EPSILON);
  writeFileSync(path, `${[header(), assistant("before", 25, .3), summary("compact", .4)].map(JSON.stringify).join("\n")}\n`);
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { estimatedUsd: .7 });
}));

test("a compaction-time unknown retains the prior valid occupancy without retaining stale cost", () => withSession([
  header(), assistant("before", 25, .3), summary("compact", .4),
], path => {
  const snapshot: AgentTelemetry = { version: 1, origin: origin(path), contextTokens: 25, estimatedUsd: .3 };
  assert.deepEqual(refreshTelemetry(snapshot), { version: 1, origin: origin(path), contextTokens: 25, estimatedUsd: .7 });
}));
...
test("reported zero is retained, while an empty history has no invented usage or cost", () => withSession([header(), assistant("zero", 0, 0)], path => {
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 0, estimatedUsd: 0 });
  writeFileSync(path, `${JSON.stringify(header())}\n`);
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), {});
}));

test("a known call with absent or invalid cost makes the complete total unknown without discarding valid context occupancy", () => withSession([
  header(), assistant("known", 20, .2), assistant("uncosted", 30),
], path => {
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 30, partialEstimatedUsd: .2 });
  writeFileSync(path, `${[header(), assistant("invalid", 31, -1)].map(JSON.stringify).join("\n")}\n`);
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 31 });
}));
...
test("usage-bearing tool-result entries are attributable even though usage is nested in message", () => withSession([
  ...
  assert.deepEqual(reduceTelemetry(origin(path), readSessionEntries(path)), { contextTokens: 5, estimatedUsd: .375 });
}));
...
test("malformed interior and incomplete trailing JSONL retain a prior snapshot instead of publishing partial totals", () => withSession([
  ...
  const snapshot: AgentTelemetry = { version: 1, origin: origin(path), contextTokens: 7, estimatedUsd: .7 };
...
test("a complete newer read retains last-valid context while clearing stale cost, and path/session/anchor mismatches are unknown", () => withSession([
  header(), assistant("old", 8, .8),
], path => {
  const snapshot: AgentTelemetry = { version: 1, origin: origin(path), contextTokens: 8, estimatedUsd: .8 };
  writeFileSync(path, `${JSON.stringify(header())}\n`);
  assert.deepEqual(refreshTelemetry(snapshot), { version: 1, origin: origin(path), contextTokens: 8 });
