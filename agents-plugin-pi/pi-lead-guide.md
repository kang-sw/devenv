...
| Run one short command yourself when you need its output inline right now | `do-i-really-have-to-run-this-myself` (`command`, `why`). Fixed 30s timeout and 4KB output cap (trimmed to the last complete line, with a hint if truncated) — never yours to raise. The name is the point: single short command, nothing multi-step/long-running/mutating. Anything wider goes through `ws-execute`. |
