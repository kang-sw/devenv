...
- Otherwise: call `{{.McpNamespace}}/git.merge_base(base: "main", head: "HEAD")` and use `<merge-base>..HEAD`.

### 3. Scan commits

Call `{{.McpNamespace}}/git.log(range: "<range>", include_body: true)`. Apply **judge: spec-impact** to each commit.
...
2. Commit through `{{.McpNamespace}}/git.commit(paths: ["ai-docs/spec/"], title: "docs(spec): ...", ai_context: ["<summary>"])`.
