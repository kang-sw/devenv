...
- Before implementing, load relevant mental-model docs; use `{{.McpNamespace}}/mental_models.find` or `{{.McpNamespace}}/mental_models.status` when available.
...
report when none exist. Skip if no test suite exists. Read the full output of
