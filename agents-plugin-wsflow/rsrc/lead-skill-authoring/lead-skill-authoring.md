...
2. When a mirrored surface includes `agents-plugin-wsflow`, read `ai-docs/ref/wsflow-mirroring.md` first and follow its required checks — its `rsrc/` tree is a generated byte-identical mirror; regenerate it with the documented test command, never hand-edit it.
