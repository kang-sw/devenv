...
Call `wsflow/mental_models.list()` to load the current mental-model catalog.
...
3. Use the `wsflow/mental_models.list()` result for the current domain catalog and hierarchy.
...
user asks - `wsflow:lead-add-rule` is a rule authoring skill, not a mental-model
scaffolding skill. `wsflow:lead-forge-mental-model` owns full-doc authorship.

## Doctrine

`wsflow:lead-add-rule` optimizes for **classification accuracy at capture time**.
