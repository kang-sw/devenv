...
- Phase 1 commits staged files only through `wsflow/git.status` and `wsflow/git.commit`; exclude `ai-docs/_index.md`.
...
1. Call `wsflow/git.status()` to identify staged files.
...
Call `wsflow/git.commit(paths: ["ai-docs/_index.md"], title: "chore(session): exit context note", ai_context: ["Session context note; no design decisions."])`.
