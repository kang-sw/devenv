...
description: Run a premise-gated, brief-mediated critical design review with fresh reviewer isolation.
---
...
1. Run discussion verification first by invoking `wsflow:lead-verify-discussion` when available; if unavailable, locally identify corrected premises, evidence, constraints, reuse opportunities, over-alignment risks, countercases, and unresolved unknowns.
...
1. Call `wsflow/path.generate(kind: "review", stems: ["verify-design-brief", "verify-design-review"])`; capture `<brief-path>` and `<review-path>`; if path generation fails, report failure and stop.
...
1. When a fresh isolated reviewer is available, invoke it with a deep or reasoning-capable model.
2. Give the reviewer only `<brief-path>`, `<review-path>`, and **Design Reviewer prompt**.
3. If no fresh isolated reviewer is available, perform the same review locally and label isolation as degraded.
...
- Another session, reviewer, ticket, or spec will need the review result.
