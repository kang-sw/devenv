...
- Move status with `ws/tickets.move` / `ws/tickets.close`; use `git mv` only when
...
- **Repo identity.** Meta-workflow repository for workflow documents, skills,
  ...
  package: `agents-plugin/` (`ws@0.46.18`). Agentless
  derivative package: `agents-plugin-wsflow/` (`wsflow@0.46.18`). Native
