...
5. **Explore when needed**: Use Grep/Glob/Read for focused queries within the allowed file set. For broader or external lookups, use `bash ~/.claude/infra/ask.sh "<question>"` (haiku) or `bash ~/.claude/infra/ask.sh --deep-research "<question>"` (sonnet). Do not read files outside `Allowed files` without lead authorization.
