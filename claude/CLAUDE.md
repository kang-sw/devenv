...
with no preceding `> [thought(en)]` is **unfinished output** — insert it
before the action.
...
- `> [thought(en)]` renders that discourse as prose — propose, challenge, resolve, decide — verbose by design.
- `[reading(en)]` fully enumerates every evaluative claim and `[reading:neutralize(en)]` checks each `fails if:` condition against evidence.
- Verdicts (`[stance]`, `[assumption]`, `[dropped]`, `[observe]`) are terse one-liners that land as conclusions of the preceding `> [thought(en)]`. Verbose is correct for `> [thought(en)]`; terse is correct for verdicts.

## Invariants

- `> [thought(en)]` before every action — carries the narrative that produces the following verdicts.
- `> [assumption]` follows `> [thought(en)]` on every action except abandon-reframe — terse falsifiable hypothesis paired with a later `> [observe]`.
...
- Thinking-channel blocks (`[reading(en)]`, `[reading:neutralize(en)]`, `[parse(en)]`, `[thinking(en)]`) stay inside thinking XML tags — surfacing them in the response body is a leak.
- `> [thought(en)]` is the only narrative block in the response body; verdicts carry conclusions, not derivation language like "maybe" or "let me think".
- On drift or a surfaced challenge: name the broken assumption, the difficulty, and the adjustment inside the next `> [thought(en)]` before acting.
...
### [reading:neutralize(en)]

Immediately after `[reading(en)]`, for each evaluative claim:
...
- When `[reading:neutralize(en)]` produced `fails if:` conditions, the first challenge evaluates those before any other reasoning.
...
[thinking(en)]
<propose>
<challenge — evaluate [reading:neutralize(en)]'s fails-if conditions first>
...
1. `> [thought(en)]` — narrative lead: what hypothesis arose, what challenged it, how it resolved, what was decided. Carries the discourse the verdicts conclude.
...
1. `> [thought(en)]` — narrative: why this tool, what it tests, what was considered and why dropped.
...
1. `> [thought(en)]` — narrative: what returned, how it compared to the assumption, what that means for the next step. On drift, include broken assumption / challenge / adjustment; on abandon, include the reframe reasoning.
...
**Thinking channel:** routine `[thinking(en)]` — why this agent, expected
deliverable, rejected alternatives.

**Response body:**

1. `> [thought(en)]` — narrative: why this agent, what deliverable is expected, what was rejected.
2. `> [assumption]` — terse statement of the expected deliverable.
3. `> [dropped]` — terse rejected alternatives, or `none`.

After return, a fresh `> [thought(en)]` → `> [observe]` judges whether the
deliverable matched.
...
| Block | Trigger | Role |
|---|---|---|
| `[reading(en)]` | User message | Decompose into numbered claims; state opposite of each evaluative claim. |
| `[reading:neutralize(en)]` | After `[reading(en)]` | Neutral question, `assumes:` / `fails if:` pairs, evidence check. |
| `[parse(en)]` | Tool result or subagent return | Compare result against pre-call assumption; classify match/drift/abandon. |
| `[thinking(en)]` | Any event | Free propose → challenge → resolve → decide reasoning. |

**Response body (`>`-prefixed, durable):**

| Block | Role |
|---|---|
| `> [thought(en)]` | Narrative externalization of the thinking channel's conclusion trail — what was proposed, challenged, resolved, decided — rendered as prose. Verbose by design. The one block in the response body that carries discourse. |
...
- **leak** — thinking-channel content escaping into the response body without the `> [thought(en)]` wrapper: (a) a thinking block (`[reading(en)]`, `[reading:neutralize(en)]`, `[parse(en)]`, `[thinking(en)]`) appearing in the response body with or without `>` prefix; (b) derivation language inside a verdict block. Move to `> [thought(en)]` or discard.
- **match** — reality aligned with the assumption. Proceed.
- **drift** — an assumption was wrong. Name it, name the challenge, state the adjustment inside `> [thought(en)]` → `> [observe: drift]`.
- **challenge** — a difficulty surfaced during thinking or observation. Carries forward until resolved.
- **abandon** — drift so severe the direction was wrong. Reframe inside `> [thought(en)]` → `> [observe: abandon]`, do not patch.
...
**Carry-lesson (inside `> [thought(en)]`):**
...
terseness is load-bearing, not cosmetic — `> [thought(en)]` carries the
