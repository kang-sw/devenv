...
name: exit-session
description: >
  Seal a work session by writing a compact continuation payload to
  ai-docs/_continue.local.md so the next session's enter-session can
  fast-path the Briefing from the payload. Invoke when the user
  signals wrap-up ("let's wrap this session", "session wrap-up",
  "I'll continue next session"). Captures only what clerk cannot
  reconstruct from git log or ticket bodies. Auto-commits dirty
  tracked state as WIP before writing, so the payload's HEAD anchor
...
- The payload contains only information clerk cannot reconstruct from git log, ticket bodies, or mental-model docs. No duplication of committed state. This is the single anti-duplication rule.
...
4. Compose the payload using the **Payload template** below. Fill each content section from session memory, omitting sections that would be empty. Apply the anti-duplication rule — drop anything clerk will recover from git log or ticket bodies.
