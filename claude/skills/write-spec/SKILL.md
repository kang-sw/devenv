...
3. If updating an existing spec:
   ...
   b. Apply changes — add 🚧 headings or body anchors. For each new anchor, run `generate-spec-stem <descriptive-slug>` first to obtain a collision-free `{#YYMMDD-slug}`. Anchors may appear on any line, not heading-only. Add Planned callouts, remove 🚧 from confirmed-implemented features.
   c. Run `spec-build-index` to regenerate the `features:` frontmatter field.
