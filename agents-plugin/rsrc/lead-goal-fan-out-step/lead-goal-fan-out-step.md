...
**Board.** Keep a one-line `{{.McpNamespace}}/session.note(session_key: <your key>, child_session_key: <worker key>, text: ...)` per worker, and read `{{.McpNamespace}}/session.children(session_key: <your key>)` to re-find workers after your own context compaction. It is a self-reported scratchpad, not completion tracking (the host signal above is that), and it is what keeps in-flight tickets out of the next selection — keep it current as workers progress.
