...
- Template sources are `AGENTS.template.md` and `WORKFLOW.template.md` in this skill directory; read both before any action.
...
1. Read `AGENTS.template.md` and `WORKFLOW.template.md`.
...
3. Create `ai-docs/` structure per the template setup block, including `ai-docs/workflow/`.
4. Copy `WORKFLOW.template.md` to `ai-docs/workflow/WORKFLOW.md`.
...
5. Ensure `ai-docs/workflow/WORKFLOW.md` exists from `WORKFLOW.template.md`; if a project-local guide already exists, preserve project additions and only merge missing bootstrap semantics.
