...
Ticket Paths
- Existing non-epic `ready/` ticket path skips `ws:lead-write-ticket` unless the freshness gate fires, then selects an implementation slice.
...
8. If `judge: ticket-freshness` found missing settled decisions, invoke `ws:lead-write-ticket` edit and append:
   `Chained from ws:lead-proceed - refresh this ticket from active conversation context only; capture settled decisions, constraints, and rejected alternatives that are missing from the ticket; do not inspect source code, read broad documentation, rejudge decomposition, or plan implementation.`
   Capture the refreshed ticket path.
9. Use only non-epic `ready/` ticket paths downstream.
10. Apply `judge: implementation-slice`.
11. Build pipeline: `ws:lead-implement`.
