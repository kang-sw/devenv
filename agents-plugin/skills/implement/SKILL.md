...
**Direct-edit:** Invoke `ws:edit` with the target as arguments.

**Delegated:** Create `implement/<scope>` branch. Invoke `ws:write-code` with the target as arguments.

Capture the commit range from the skill's completion report.

### 4. Doc pre-pass

1. Invoke `ws:update-spec` with args `<commit-range>`. Lead-driven; runs inline.
