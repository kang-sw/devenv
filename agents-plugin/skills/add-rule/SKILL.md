...
user asks - `ws:add-rule` is a rule authoring skill, not a mental-model
scaffolding skill. `ws:forge-mental-model` owns full-doc authorship.

## Doctrine

`ws:add-rule` optimizes for **classification accuracy at the moment a rule
